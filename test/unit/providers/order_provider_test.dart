...
void main() {
  ...
  group('OrderProvider', () {
    ...
    Future<OrderProvider> buildProvider() async {
    ...
    group('placeOrder', () {
      test('adds a new order to pastOrders', () async {
        final provider = await buildProvider();
        await provider.placeOrder(sampleItems(), 31.50);
      ...
      test('placing multiple orders grows the list', () async {
        final provider = await buildProvider();
        await provider.placeOrder(sampleItems(), 10.00);
        await provider.placeOrder(sampleItems(), 20.00);
